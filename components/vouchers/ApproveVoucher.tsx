...
import type { TravelExpense } from "../requisitions/TravelExpensePage";
import type { RentEntry } from "../requisitions/rentTypes";
...
interface ApproveReqPageProps {
  mode?: "review" | "approve-expense-vouchers";
  themeColor?: string;
  ...
  const filteredRows = useMemo(
    () =>
      effectiveRows.filter((row) => {
        // Base filter for approve-expense-vouchers mode: Only show Approved records with no expense voucher created
        if (mode === "approve-expense-vouchers") {
          if (
            normalizeStatus(row.status) !== "approved" ||
            (row.expenseVoucherCreated || "-") !== "-"
          ) {
            return false;
          }
        }
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-black tracking-tight uppercase">
                  {mode === "approve-expense-vouchers" ?
                    "Approve Expense Vouchers"
                  : "Review Requisitions"}
                </h1>
                <p className="text-white/80 text-xs font-medium mt-1">
                  {mode === "approve-expense-vouchers" ?
                    `${currentMonthName} approved requisitions pending expense voucher creation.`
                  : `${currentMonthName} queue across Consultant, Rent, and Travel requisitions.`
                  }
                </p>
              </div>
...
// ============================================================================
// VOUCHER APPROVAL COMPONENT (for Expense/Bank Vouchers)
// ============================================================================

interface ApproveVoucherPageProps {
  type: "Expense" | "Bank";
  themeColor?: string;
  voucherData?: VoucherRecord[];
  onApproveVoucher?: (voucherId: string, createBankVoucher?: boolean) => void;
  onRejectVoucher?: (voucherId: string, reason: string) => void;
}

export const ApproveVoucherPage: React.FC<ApproveVoucherPageProps> = ({
  type,
  themeColor = "brand-600",
  voucherData = [],
  onApproveVoucher,
    ...
    () =>
      voucherData.filter(
        (v) =>
          ...
          (v.date || "").toLowerCase().includes(filterDate.toLowerCase()) &&
          (v.project || "")
            .toLowerCase()
            .includes(filterProject.toLowerCase()) &&
          (v.status || "").toLowerCase().includes(filterStatus.toLowerCase()),
      ),
    [
      voucherData,
      filterId,
  ...
  const pendingCount = voucherData.filter(
    (v) => (v.status || "").toLowerCase() === "pending review",
  ).length;
  ...
  const openBulkApprove = () => {
    ...
    setApproveModal({
      ...
      remarks: "",
      createBankVoucher: false,
    });
  };

  const openBulkReject = () => {
    if (selectedIds.size === 0) {
      alert("Please select at least one voucher to reject.");
      return;
    }
    setRejectModal({
      ids: Array.from(selectedIds),
  ...
  const confirmApprove = () => {
    if (!approveModal) return;
    approveModal.ids.forEach((id) => {
      if (onApproveVoucher) {
        onApproveVoucher(
          id,
          type === "Expense" && approveModal.createBankVoucher,
        );
      }
    });
    setSelectedIds(new Set());
  ...
  const confirmReject = () => {
    ...
    rejectModal.ids.forEach((id) => {
      if (onRejectVoucher) {
        onRejectVoucher(id, rejectModal.remarks);
      }
    });
    setSelectedIds(new Set());
    setRejectModal(null);
  ...
  const hasActiveFilters =
    filterId || filterPayee || filterDate || filterProject || filterStatus;

  const mainLabel = type === "Expense" ? "Expense Voucher" : "Bank Voucher";
          ...
          <div className={`bg-${themeColor} px-8 py-6 text-white`}>
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-white/80 text-xs font-medium mt-1">
                  Review and authorize {type.toLowerCase()} voucher
                  disbursements.
                </p>
              </div>
              <div className="text-right">
              ...
              <table className="w-full text-left border-collapse">
                  ...
                  <tr className={`bg-${themeColor} text-white`}>
                    <th className="p-4 w-12">
                      <input
                        ...
                        checked={
                          ...
                          selectedIds.size === filteredVouchers.length
                        }
                        onChange={toggleSelectAll}
                    ...
                    <th className="p-4">
                      ...
                      <input
                        ...
                        className="w-full px-2 py-1 rounded text-xs text-slate-900 font-semibold"
                      />
                    </th>
                    <th className="p-4">
                      <div className="text-[10px] font-black uppercase tracking-widest mb-2">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredVouchers.length === 0 ?
                    <tr>
                      <td
                        colSpan={type === "Bank" ? 9 : 8}
                        className="p-8 text-center text-slate-400 text-sm font-semibold">
                        No {type.toLowerCase()} vouchers found matching your
                        filters.
                      </td>
                    </tr>
                      ...
                      <tr
                        ...
                        <td className="p-4">
                          <button
                            ...
                            className="text-brand-600 hover:text-brand-700 font-black text-xs underline">
                            {voucher.id}
                          </button>
                          {voucher.linkedReq && (
                            <div className="text-[10px] text-slate-400 mt-0.5">
                              Req: {voucher.linkedReq}
                            </div>
                          )}
                        </td>
                        ...
                        <td className="p-4 text-xs font-black text-slate-900 dark:text-white">
                          ₹{(voucher.amount || 0).toLocaleString("en-IN")}
                        </td>
                        {type === "Bank" && (
                          <td className="p-4 text-xs font-semibold text-slate-700 dark:text-slate-300">
                            {voucher.linkedExpenseVoucher || "-"}
                          </td>
                        )}
                        <td className="p-4">
                          <span
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition shadow-sm">
                              Approve
                            </button>
                            <button
                              onClick={() => {
                              ...
                              className="px-3 py-1.5 bg-rose-500 hover:bg-rose-600 text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition shadow-sm">
                              Reject
                            </button>
                          </div>
                        </td>
        ...
        <div
          ...
          <div
            ...
            <div className="bg-emerald-500 px-6 py-5">
              ...
              <p className="text-white/80 text-xs font-semibold mt-1">
                You are about to approve {approveModal.ids.length} item(s).
              </p>
            </div>
            <div className="p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
              ...
              {type === "Expense" && (
                <label className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-violet-50/40 transition">
                  <input
                    type="checkbox"
                    checked={approveModal.createBankVoucher}
                    onChange={(e) =>
                      setApproveModal((prev) =>
                        prev ?
                          { ...prev, createBankVoucher: e.target.checked }
                        : prev,
                      )
                    }
                    className="w-4 h-4 rounded accent-violet-600"
        ...
        <div
          ...
          <div
            ...
            <div className="bg-rose-500 px-6 py-5">
              ...
              <p className="text-white/80 text-xs font-semibold mt-1">
                You are about to reject {rejectModal.ids.length} item(s).
              </p>
            </div>
            <div className="p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
...
// Export ApproveReqPage as ApproveVoucher for backward compatibility
export const ApproveVoucher = ApproveVoucherPage;
