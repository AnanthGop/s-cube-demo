...
interface ApprovePaymentVoucherPageProps {
  themeColor?: string;
  data?: ExpenseVoucherRow[];
  onCreatePaymentVoucher?: (reqId: string, sourceType: string) => void;
...
export const ApprovePaymentVoucherPage: React.FC<
  ApprovePaymentVoucherPageProps
> = ({
  ...
  // Filter data: status = 'Approved' AND expenseVoucherCreated = 'Y' AND paymentVoucherCreated = '-'
  ...
  const confirmReject = () => {
    ...
    console.log(`Rejecting payment voucher for ${rejectModal.reqId}`);
  ...
  const handleBulkCreateVouchers = () => {
    if (selectedRows.size === 0) {
      alert("Please select at least one entry to create payment vouchers.");
      return;
    }
    const confirmed = window.confirm(
      `Are you sure you want to create payment vouchers for ${selectedRows.size} selected item(s)?`,
    );
    if (!confirmed) return;

    selectedRows.forEach((reqId) => {
      const row = filteredRows.find((r) => r.reqId === reqId);
      if (row && onCreatePaymentVoucher) {
        onCreatePaymentVoucher(row.reqId, row.sourceType);
      }
    });
    setSelectedRows(new Set());
  };
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-white/80 text-xs font-medium mt-1">
                  {currentMonthName} - Approved expense vouchers ready for
              ...
              {selectedRows.size > 0 && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    <span>✓</span>
                    Create Payment Vouchers ({selectedRows.size})
                  </button>
                </div>
              )}
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-100 to-slate-50 text-[9px] font-black text-slate-500 uppercase tracking-widest border-b-2 border-slate-300">
                    ...
                    <th className="px-4 py-4 min-w-[220px]">Bank Account</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      ...
                      return (
                        <tr
                          ...
                          <td className="px-4 py-3 min-w-[220px]">
                            <select
                              value={selectedBankAccounts[row.reqId] || ""}
                              onChange={(e) =>
                                setSelectedBankAccounts((prev) => ({
                                  ...prev,
                                  [row.reqId]: e.target.value,
                                }))
                              }
                              className="w-full min-w-[220px] px-2 py-1.5 bg-white border border-slate-300 rounded text-xs font-semibold outline-none focus:ring-2 focus:ring-brand-500/30">
                  ...
                  : <tr>
                      <td
                        ...
                        No approved expense vouchers awaiting payment voucher
                        creation.
                      </td>
                    </tr>
                  }
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/30 backdrop-blur-sm animate-in fade-in duration-300">
          <div
            ...
            <div className="bg-emerald-600 px-6 py-5">
              <h2 className="text-xl font-black tracking-tight uppercase text-white">
                Approve for Payment Voucher
              </h2>
              <p className="text-white/80 text-xs font-semibold mt-1">
                You are about to approve 1 item(s) for payment voucher creation.
              </p>
            </div>
            {/* Body */}
            <div className="p-6 flex flex-col gap-5">
