...
interface DepreciationMastersPageProps {
...
export default function DepreciationMastersPage({
  fyList,
  assetTypes,
  depreciationRatesIT,
  depreciationRatesCA,
  ...
  const saveIT = () => {
    if (!selectedFY || !formIT.assetType || !formIT.depreciationRate) {
  ...
  const saveCA = () => {
    if (!selectedFY || !formCA.assetType || !formCA.usefulLife || !formCA.salvageValue) {
      window.alert('Please fill all required fields.');
      return;
    }
    ...
    if (salvage < 0 || salvage > 100) {
      window.alert('Salvage Value (%) must be between 0 and 100.');
      return;
    }
