...
interface ConsultantMapToCoaPageProps {
  themeColor?: string;
...
export const ConsultantMapToCoaPage: React.FC<ConsultantMapToCoaPageProps> = ({
  ...
  const [journalEntryConfigs, setJournalEntryConfigs] = useState<{
    ...
    expenseVoucher: [
      {
        ledger: "Consultant Fees",
        consultant: "",
        type: "Debit",
        showTds: false,
      },
      {
        ledger: "<Consultant Details>",
        consultant: "",
        type: "Credit",
        showTds: false,
      },
    ...
    paymentVoucher: [
      ...
      { ledger: "Bank", consultant: "", type: "Credit", showTds: false },
    ],
  });

  const uniqueConsultantRecords = useMemo(() => {
    const byConsultant = new Map<string, ConsultantRecord>();
    consultantRecords.forEach((entry) => {
      const consultantName = String(entry?.consultantName || "").trim();
      if (!consultantName || byConsultant.has(consultantName)) return;
      byConsultant.set(consultantName, { ...entry, consultantName });
    });
    return Array.from(byConsultant.values()).sort((a, b) =>
      String(a.consultantName || "").localeCompare(
        String(b.consultantName || ""),
      ),
    );
  }, [consultantRecords]);

  const consultantDetailsLookup = useMemo(() => {
    const lookup = new Map<string, ConsultantRecord>();
    uniqueConsultantRecords.forEach((record) => {
      const key = String(record.consultantName || "");
      if (key) lookup.set(key, record);
    });
    return lookup;
  }, [uniqueConsultantRecords]);
  ...
  const handleApplyTdsAccount = async () => {
    ...
    const nextData = [
      {
        accountName: `TDS Account (${String(selectedRule.section || "194J")})`,
  ...
  const handleAddEntry = () => {
    ...
    const newEntry = {
      ...
      consultant: selectedConsultantDetail,
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          value={selectedConsultantDetail}
                          onChange={(e) =>
                            setSelectedConsultantDetail(e.target.value)
                          }
                          className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:ring-1 focus:ring-indigo-500 outline-none bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100">
                          <option value="">None</option>
                          <option value="<Consultant Details>">
                            &lt;Consultant Details&gt;
                          </option>
                        </select>
                      </div>
                    ...
                    <div className="mt-4 flex justify-end">
                      <button
                        ...
                        disabled={!selectedLedger && !selectedConsultantDetail}
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {tdsAccountMappings.length > 0 ?
                      tdsAccountMappings.map((item, idx) => (
                        <tr
                          key={`${item.accountName}-${item.section}-${idx}`}
                          className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                          <td className="px-6 py-4 text-sm text-slate-800 dark:text-slate-100">
